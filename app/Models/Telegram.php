...
class Telegram
{
    ...
    {
        $url = config('telegram.urls.main') . ':8081/bot'.$this->bot_token.'/'.$api;
        if ($post) {
    ...
    public function endpointdash($api, array $content, $post = true)
    {
    ...
    public function getFileDash($file_id)
    {
        $content = ['file_id' => $file_id];

        return $this->endpointdash('getFile', $content);
    }
