@php
    $disabled = $errors->any() || empty($this->task->lesson_id) || empty($this->task->description) ? true : false;
@endphp
    ...
    <div class="row w-100 h-100 p-0 mx-0">
        <div class="col-md-8">
                ...
                <div class="block-content">
                    <div class="col-12">
                        <div class="row w-100 h-100 p-0 m-0">
                            ...
                            <div class="col-md-6 pe-0">
                                @if(!is_null($lessons) or isset($task->lesson_id))
                                    <div class="mb-4">
                                        <label class="form-label" for="lesson_id">Lesson</label>
                                        <select class="form-select" id="lesson_id" name="lesson_id" wire:model="task.lesson_id">
                                            <option value="null" selected disabled>Choose lesson</option>
                    ...
                    <div class="col-12">
                        <div class="mb-4" wire:ignore>
                            ...
                            <trix-editor input="description" wire:model.debounce.500ms="task.description"></trix-editor>
            ...
            <div class="mb-4 d-md-block d-none">
                <button wire:target="update" wire:loading.attr="disabled" @if($disabled){{ 'disabled' }}@endif type="submit" class="btn bg-primary text-white px-4">Save</button>
                <a href="{{ route('tasks.index') }}" class="btn border bg-white px-4">Cancel</a>
            </div>
        </div>
        <div class="col-md-4">
            <div class="block block-rounded">
                ...
                <div class="block-content">
                    <div class="mb-4">
                        <div x-data="{ isUploading: false, progress: 0 }"
                            ...
                            <label class="form-label" for="file">Task media</label>
                        ...
                        <span class="text-danger pt-2 ps-1 small">{{ $message }}</span>
                        @enderror
                        @if(isset($task->file) and !$file)
                            <div class="file-block pt-3">
                                {!! $task->getFile() !!}
                            </div>
                        @endif
                    ...
                    <div class="mb-4">
                        ...
                        <div class="space-x-2">
                            <div class="form-check form-switch form-check-inline">
                                <input  wire:model="task.active" class="form-check-input @error('active'){{ 'is-invalid' }}@enderror" type="checkbox" value="1" id="active" name="active" checked="">
                        ...
                        @if($additional_files > 0)
                            @for($i = 0; $i < $additional_files; $i++)
                                <div class="mb-4">
                                    ...
                                    <span class="text-danger pt-2 ps-1 small">{{ $message }}</span>
                                    @enderror
                                    @if(isset($task->files[$i]) and !isset($files[$i]))
                                        <div class="file-block pt-3">
                                            {!! $task->files[$i]->getFile() !!}
                                        </div>
                                    @endif
                                    @if(isset($files[$i]))
                                        @php
...
@push('scripts')
    <script>
        ...
        @this.set('task.description', trixEditor.getAttribute('value'))
        })
    </script>
@endpush
