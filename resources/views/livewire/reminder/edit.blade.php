@php
    $disabled = $errors->any() || empty($this->reminder->per_day);
@endphp
    ...
    <div class="row w-100 h-100 p-0 mx-0">
        <div class="col-md-8">
                ...
                <div class="block-content">
                    <div class="col-12">
                        <div class="mb-4">
                            ...
                            <select wire:model="reminder.per_day" name="perDay" id="perDay"
                                    class="form-select @error('reminder.per_day'){{ 'is-invalid' }}@enderror">
                        ...
                        <div class="mb-4" wire:ignore>
                            ...
                            <trix-editor input="text" wire:model.debounce.500ms="reminder.text"></trix-editor>
                            @error('reminder.text')
                            <span class="text-danger pt-2 ps-1 small">{{ $message }}</span>
                            @enderror
                        </div>
            ...
            <div class="mb-4 d-md-block d-none">
                <button wire:target="update" wire:loading.attr="disabled" @if($disabled)
                    {{ 'disabled' }}
            ...
            <div class="block block-rounded">
                ...
                <div class="block-content">
                    <div class="mb-4">
                        ...
                        <span class="text-danger pt-2 ps-1 small">{{ $message }}</span>
                        @enderror
                        @if(isset($reminder->file) and !$file)
        ...
        <div class="mb-4 d-md-none d-block">
            <button wire:target="update" wire:loading.attr="disabled" @if($disabled){{ 'disabled' }}@endif type="submit" class="btn bg-primary text-white px-4">Save
            </button>
            <a href="{{ route('reminders.index') }}" class="btn border bg-white px-4">Cancel</a>
        </div>
    </div>
...
@push('scripts')
    <script>
        ...
        @this.set('reminder.text', trixEditor.getAttribute('value'))
        })
    </script>
@endpush
